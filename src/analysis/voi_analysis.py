...
# Stakeholder archetype weight configurations
ARCHETYPES = {
    "privacy_first": {
        "name": "Privacy-First",
        "weights": {"fidelity": 0.10, "privacy": 0.50, "utility": 0.20, "fairness": 0.10, "efficiency": 0.10}
    },
    "utility_first": {
        "name": "Utility-First",
        "weights": {"fidelity": 0.20, "privacy": 0.10, "utility": 0.50, "fairness": 0.10, "efficiency": 0.10}
    },
    "balanced": {
        "name": "Balanced",
        "weights": {"fidelity": 0.25, "privacy": 0.25, "utility": 0.25, "fairness": 0.15, "efficiency": 0.10}
    }
}

# Method labels
METHOD_LABELS = {
    'independent_marginals': 'Ind. Marginals',
    'ctgan': 'CTGAN',
    'dpbn': 'DP BN',
    'synthpop': 'Synthpop',
    'great': 'GReaT',
...
METRIC_DIRECTION = {
    "fidelity_auc": False,  # Lower AUC = better fidelity
    "privacy_dcr": True,    # Higher DCR = better privacy
    "utility_tstr": True,   # Higher TSTR = better utility
...
def compute_method_value(values: pd.DataFrame, weights: Dict[str, float]) -> pd.Series:
    ...
    # Add efficiency (not in METRIC_MAP but in weights)
...
def compute_oracle(
    ...
    for _ in range(n_simulations):
        ...
        # Convert to values
        values = normalize_to_value(sampled_means)
...
def format_results_tables(results: List[VOIResults]) -> Tuple[str, str]:
    ...
    strategy_rows.append(f"S1 (Random) & {results[0].s1_random:.2f} & {results[1].s1_random:.2f} & {results[2].s1_random:.2f} & {s1_mean:.2f} \\\\")
    strategy_rows.append(f"S2 (Heuristic) & {results[0].s2_heuristic:.2f} & {results[1].s2_heuristic:.2f} & {results[2].s2_heuristic:.2f} & {s2_mean:.2f} \\\\")
    strategy_rows.append(f"S3 (Rough Estimates) & {results[0].s3_rough:.2f} & {results[1].s3_rough:.2f} & {results[2].s3_rough:.2f} & {s3_mean:.2f} \\\\")
    strategy_rows.append(f"S4 (Full Benchmark) & {results[0].s4_benchmark:.2f} & {results[1].s4_benchmark:.2f} & {results[2].s4_benchmark:.2f} & {s4_mean:.2f} \\\\")
    strategy_rows.append("\\midrule")
    strategy_rows.append(f"Oracle (EVPI bound) & {results[0].oracle:.2f} & {results[1].oracle:.2f} & {results[2].oracle:.2f} & {oracle_mean:.2f} \\\\")
    ...
    voi_rows.append(f"Value of Prioritization $V(\\text{{S3}})-V(\\text{{S1}})$ & {results[0].value_of_prioritization:.2f} & {results[1].value_of_prioritization:.2f} & {results[2].value_of_prioritization:.2f} & {vop_mean:.2f} \\\\")
    voi_rows.append(f"Value of Information $V(\\text{{S4}})-V(\\text{{S3}})$ & {results[0].value_of_information:.2f} & {results[1].value_of_information:.2f} & {results[2].value_of_information:.2f} & {voi_mean:.2f} \\\\")
    voi_rows.append(f"Total Improvement $V(\\text{{S4}})-V(\\text{{S1}})$ & {results[0].total_improvement:.2f} & {results[1].total_improvement:.2f} & {results[2].total_improvement:.2f} & {total_mean:.2f} \\\\")
    voi_rows.append("\\midrule")
    voi_rows.append(f"\\% from Prioritization & {results[0].pct_from_prioritization:.0f}\\% & {results[1].pct_from_prioritization:.0f}\\% & {results[2].pct_from_prioritization:.0f}\\% & \\textbf{{{pct_prior_mean:.0f}\\%}} \\\\")
    voi_rows.append(f"\\% from Information & {results[0].pct_from_information:.0f}\\% & {results[1].pct_from_information:.0f}\\% & {results[2].pct_from_information:.0f}\\% & {pct_info_mean:.0f}\\% \\\\")
...
def print_report(results: List[VOIResults]) -> None:
    ...
    print("-" * 70)
    print(f"{'Strategy':<25} {'Privacy-First':>12} {'Utility-First':>14} {'Balanced':>10} {'Mean':>8}")
    print("-" * 70)
    ...
    print(f"{'S1 (Random)':<25} {s1_vals[0]:>12.2f} {s1_vals[1]:>14.2f} {s1_vals[2]:>10.2f} {np.mean(s1_vals):>8.2f}")
    print(f"{'S2 (Heuristic)':<25} {s2_vals[0]:>12.2f} {s2_vals[1]:>14.2f} {s2_vals[2]:>10.2f} {np.mean(s2_vals):>8.2f}")
    print(f"{'S3 (Rough Estimates)':<25} {s3_vals[0]:>12.2f} {s3_vals[1]:>14.2f} {s3_vals[2]:>10.2f} {np.mean(s3_vals):>8.2f}")
    print(f"{'S4 (Full Benchmark)':<25} {s4_vals[0]:>12.2f} {s4_vals[1]:>14.2f} {s4_vals[2]:>10.2f} {np.mean(s4_vals):>8.2f}")
    print("-" * 70)
    print(f"{'Oracle (EVPI bound)':<25} {oracle_vals[0]:>12.2f} {oracle_vals[1]:>14.2f} {oracle_vals[2]:>10.2f} {np.mean(oracle_vals):>8.2f}")
    print()
    
    # VOI decomposition
    print("VOI DECOMPOSITION")
    print("-" * 70)
    print(f"{'Component':<35} {'Privacy-First':>12} {'Utility-First':>14} {'Balanced':>10} {'Mean':>8}")
    print("-" * 70)
    ...
    print(f"{'Value of Prioritization':<35} {vop_vals[0]:>12.2f} {vop_vals[1]:>14.2f} {vop_vals[2]:>10.2f} {np.mean(vop_vals):>8.2f}")
    print(f"{'Value of Information':<35} {voi_vals[0]:>12.2f} {voi_vals[1]:>14.2f} {voi_vals[2]:>10.2f} {np.mean(voi_vals):>8.2f}")
    print(f"{'Total Improvement':<35} {total_vals[0]:>12.2f} {total_vals[1]:>14.2f} {total_vals[2]:>10.2f} {np.mean(total_vals):>8.2f}")
    print("-" * 70)
    print(f"{'% from Prioritization':<35} {pct_prior_vals[0]:>11.0f}% {pct_prior_vals[1]:>13.0f}% {pct_prior_vals[2]:>9.0f}% {np.mean(pct_prior_vals):>7.0f}%")
    print(f"{'% from Information':<35} {pct_info_vals[0]:>11.0f}% {pct_info_vals[1]:>13.0f}% {pct_info_vals[2]:>9.0f}% {np.mean(pct_info_vals):>7.0f}%")
    print()
    ...
    print(f"   (Keisler 2004 found ~71% in portfolio decision analysis)")
    print()
...
def main(filepath: str | Path = None):
    ...
    if filepath is None:
        filepath = Path(__file__).parent.parent.parent / "results/experiments/all_results_complete.csv"
