...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Testing Membership Inference Privacy Measure")
    print("=" * 60)

    # Load data
    df_train, df_test = preprocess_acs_data(sample_size=2000)

    print("\nData sizes:")
    print(f"  Train (members): {len(df_train):,}")
    print(f"  Test (non-members): {len(df_test):,}")

    # Generate synthetic data
    print("\nGenerating synthetic data with Independent Marginals...")
    df_synthetic = generate_independent_marginals(
        df_train=df_train, n_samples=len(df_train), random_state=42
    )

    # Compute membership inference privacy
    print("\nComputing membership inference privacy...")
    result = membership_inference_privacy(
        df_real_train=df_train, df_real_test=df_test, df_synthetic=df_synthetic, random_state=42
    )

    print("\nResults:")
    print(f"  Attack Success Rate: {result['attack_success_rate']:.4f}")
    print(f"  Attack AUC (on real data): {result['attack_auc']:.4f}")
    print(f"  Mean Membership Confidence: {result['mean_membership_confidence']:.4f}")
    print(f"  Privacy Score: {result['privacy_score']:.4f}")

    print("\nInterpretation:")
    print("  - Attack Success Rate = 0.5 means attacker is guessing randomly (perfect privacy)")
    print("  - Attack Success Rate = 1.0 means attacker always predicts 'member' (worst privacy)")
    print("  - Privacy Score = 1.0 means perfect privacy (attack success = 0.5)")
    print("  - Privacy Score = 0.0 means no privacy (attack success = 1.0 or 0.0)")

    print("\nCurrent results suggest:")
    if result["privacy_score"] > 0.8:
        print("  ✓ Excellent privacy - synthetic data doesn't reveal training membership")
    elif result["privacy_score"] > 0.6:
        print("  ✓ Good privacy - synthetic data has limited training memorization")
    elif result["privacy_score"] > 0.4:
        print("  ⚠ Moderate privacy - some training memorization detected")
    else:
        print("  ⚠ Poor privacy - significant training memorization")

    # Test per-record version
    print("\n" + "=" * 60)
    print("Per-Record Membership Probabilities")
    print("=" * 60)

    df_synth_with_scores = membership_inference_privacy_per_record(
        df_real_train=df_train, df_real_test=df_test, df_synthetic=df_synthetic, random_state=42
    )

    print("\nSample records with high membership probability:")
    high_prob = df_synth_with_scores.nlargest(5, "membership_prob")
    print(high_prob[["AGEP", "PINCP", "SEX", "membership_prob", "is_likely_member"]])

    print("\nSample records with low membership probability:")
    low_prob = df_synth_with_scores.nsmallest(5, "membership_prob")
    print(low_prob[["AGEP", "PINCP", "SEX", "membership_prob", "is_likely_member"]])

    print("\nDistribution of membership probabilities:")
    print(df_synth_with_scores["membership_prob"].describe())
