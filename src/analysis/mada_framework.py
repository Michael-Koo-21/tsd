...
# Pre-defined user profiles for common use cases
# NOTE: These match the stakeholder archetypes in the paper (Table 3)
PROFILES = {
    "privacy_first": UserProfile(
        name="Privacy-First Practitioner",
        description="Prioritizes data privacy and protection against re-identification",
        weights={"fidelity": 0.25, "privacy": 0.45, "utility": 0.15, "fairness": 0.10, "efficiency": 0.05}
    ),
    "utility_first": UserProfile(
        name="Utility-First Practitioner",
        description="Prioritizes downstream ML model performance",
        weights={"fidelity": 0.30, "privacy": 0.10, "utility": 0.40, "fairness": 0.10, "efficiency": 0.10}
    ),
    ...
    # Legacy profiles (4 objectives) - kept for backward compatibility
    "privacy_focused": UserProfile(
        name="Privacy-Focused (Legacy 4-obj)",
        description="Prioritizes data privacy (4-objective legacy profile)",
        weights={"fidelity": 0.15, "privacy": 0.50, "utility": 0.25, "fairness": 0.10, "efficiency": 0.00}
    ),
    "utility_focused": UserProfile(
        name="Utility-Focused (Legacy 4-obj)",
        description="Prioritizes downstream ML performance (4-objective legacy profile)",
        weights={"fidelity": 0.20, "privacy": 0.10, "utility": 0.60, "fairness": 0.10, "efficiency": 0.00}
    ),
}

# Metric to attribute mapping (5 objectives)
METRIC_MAP = {
...
# NOTE: For propensity AUC, LOWER is better (0.5 = indistinguishable = perfect fidelity)
...
METRIC_DIRECTION = {
    ...
    "privacy_dcr": True,    # Higher DCR = better privacy (further from training data)
...
# Used when runtime data is not present in results file
# GReaT: ~60 min on Google Colab T4 GPU; others: <1 min on CPU
DEFAULT_RUNTIME_MINUTES = {
    "independent_marginals": 0.1,  # ~6 seconds
    "synthpop": 0.5,              # ~30 seconds
    "ctgan": 1.0,                 # ~1 minute
    "dpbn": 0.5,                  # ~30 seconds
    "great": 60.0,                # ~60 minutes (GPU)
}
...
def generate_recommendation(
    ...
    report.append(f"Score advantage: +{(top_score - second_score):.4f} over {METHOD_LABELS.get(second_method, second_method)}")
    report.append("")
...
def plot_comparison(
    ...
    colors = {'fidelity': '#4C72B0', 'privacy': '#55A868',
              'utility': '#C44E52', 'fairness': '#8172B3', 
              'efficiency': '#CCB974'}
    ...
    for metric, attr in METRIC_MAP.items():
        ...
        ax1.barh(range(len(methods)), values, left=bottom,
                label=f"{attr.capitalize()} ({weights.get(attr, 0):.0%})",
                color=colors.get(attr, '#999999'), edgecolor='white', linewidth=0.5)
        bottom += values
    ...
    # Add total scores
    for i, (m, s) in enumerate(zip(methods, ranking['score'])):
    ...
    for method in methods:
        ...
        ax2.plot(method_data['weight'], method_data['score'],
                label=METHOD_LABELS.get(method, method), linewidth=2)

    ax2.axvline(x=weights[top_attr], color='gray', linestyle='--', alpha=0.7,
    ...
    if output_dir:
        ...
        for idx, profile_id in enumerate(primary_profiles):
            ...
            colors = ['gold' if i == 0 else 'silver' if i == 1 else 'peru' if i == 2
                     else 'lightgray' for i in range(len(methods))]

            bars = ax.barh(range(len(methods)), scores_vals, color=colors,
            ...
            # Add score labels
            for i, (bar, score) in enumerate(zip(bars, scores_vals)):
                ax.text(score + 0.02, i, f'{score:.3f}', va='center', fontsize=9)

        plt.suptitle('MADA Framework: Method Rankings by Stakeholder Archetype (5 Objectives)\n(Gold = Recommended)',
