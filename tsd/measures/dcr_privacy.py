...
def compare_dcr_distributions(
    ...
    for method_name, df_synth in synthetic_datasets.items():
        result = dcr_privacy(
            df_real_train=df_real_train, df_synthetic=df_synth, metric=metric, percentile=percentile
        )
...
if __name__ == "__main__":
    ...
    df_synth_ctgan = generate_ctgan(
        df_train=df_train, n_samples=len(df_train), epochs=30, verbose=False, random_state=42
    )
    ...
    result_indep = dcr_privacy(df_real_train=df_train, df_synthetic=df_synth_indep, percentile=5)
    ...
    print(
        f"  Close Matches (<threshold): {result_indep['num_close_matches']} ({result_indep['pct_close_matches']:.1f}%)"
    )
    print(f"  Typical Distance (in training): {result_indep['typical_distance']:.4f}")
    print(f"  Privacy Score: {result_indep['privacy_score']:.4f}")

    print("\n>>> CTGAN")
    result_ctgan = dcr_privacy(df_real_train=df_train, df_synthetic=df_synth_ctgan, percentile=5)
    ...
    print(
        f"  Close Matches (<threshold): {result_ctgan['num_close_matches']} ({result_ctgan['pct_close_matches']:.1f}%)"
    )
    ...
    if result_indep["privacy_score"] > result_ctgan["privacy_score"]:
        ...
        print("  = Both methods have similar privacy")
    ...
    comparison = compare_dcr_distributions(
        ...
        synthetic_datasets={"Independent Marginals": df_synth_indep, "CTGAN": df_synth_ctgan},
        percentile=5,
    )
    ...
    print(
        "\n"
        + comparison[
            ["method", "dcr_percentile", "dcr_mean", "dcr_median", "privacy_score"]
