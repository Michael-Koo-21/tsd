...
def pairwise_comparisons(df: pd.DataFrame, alpha: float = 0.05) -> dict:
    ...
    for metric in metrics:
        ...
        for m1, m2 in combinations(methods, 2):
            ...
            pooled_std = np.sqrt(((len(v1)-1)*v1.std(ddof=1)**2 +
...
def generate_report(df: pd.DataFrame) -> str:
    ...
    for metric, results in omnibus.items():
        report.append(f"\n{metric.upper()}:")
        report.append(f"  ANOVA: F={results['anova']['F']:.3f}, p={results['anova']['p_value']:.4f}")
        report.append(f"  Kruskal-Wallis: H={results['kruskal_wallis']['H']:.3f}, p={results['kruskal_wallis']['p_value']:.4f}")
        report.append(f"  Effect size (η²): {results['eta_squared']:.4f} ({results['effect_interpretation']})")
    ...
    for metric, results in pairwise.items():
        ...
        if sig_comparisons:
            ...
            for c in sig_comparisons:
                ...
                report.append(f"    {c['method_1']} {direction} {c['method_2']}: "
    ...
    for pair, vals in corr["spearman"].items():
        interp = "strong" if abs(vals["rho"]) > 0.7 else "moderate" if abs(vals["rho"]) > 0.4 else "weak"
    ...
    report.append(f"  • Fidelity AUC (lower=better, closer to 0.5): {desc_pivot['fidelity_auc'].idxmin()}")
